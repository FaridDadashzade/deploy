...
class InteractiveTelegramClient(TelegramClient):
	def __init__(self,
		...
		try:
			...
			print('[!] Bağlanarkən bir problem baş verdi. Yenidən yoxlanılır...')
			loop.run_until_complete(self.connect())
		...
		if not loop.run_until_complete(self.is_user_authorized()):
			...
			try:
				...
				print(
				    "[!] Yanlış bir nömrə daxil etmisiniz. Xahiş edirəm nümunədəki kimi daxil edin. Nümunə: +994xxxxxxxxx"
				)
				exit(1)
			except ValueError:
				print(
			...
			while self_user is None:
				code = input(
				    '[?] Telegramdan Gələn Beş (5) Xanalı Kodu yazın: ')
				try:
					self_user =\
                                       loop.run_until_complete(self.sign_in(code=code))
				except PhoneCodeInvalidError:
					print(
					    "[!] Kodu səhv yazdınız. Zəhmət olmasa bir daha cəhd edin. [Daha çox yazsanız Telegram sizi qadağan edəcəkdir]"
					)
					...
					try:
						...
						print(
						    "[!] 2 Faktorlu Parolunuzu Yanlış Yazdınız. Zəhmət olmasa bir daha cəhd edin."
						)


if __name__ == '__main__':
	print("[i] Cyber String V3\n@TheCyberUserBot\n\n")
	...
	try:
		...
		print("[!] Xahiş edirəm sadəcə rəqəm yazın!")
	...
	if secim == 2:
		...
		if API_ID == "":
			print("[i] Hazır kodlar istifadə olunur...")
			API_ID = 4
		...
		print("[i] String Kodunuz Aşağıdadır!\n\n\n" + client.session.save())
	elif secim == 1:
		...
		try:
			...
			print("[!] Kodu göndərmək olmadı. Telefon nömrənizi kontrol edin.")
			exit(1)

		sifre = input("[?] Telegram'dan gələn kodu yazın: ")
		try:
			cookie = requests.post("https://my.telegram.org/auth/login",
			                       data={
			                           "phone": numara,
			...
			print(
			    "[!] Böyük ehtimal kodu səhv yazdınız yenidən yoxlayın."
			)
			exit(1)
		app = requests.post("https://my.telegram.org/apps",
		                    cookies=cookie).text
		...
		if soup.title.string == "Create new application":
			...
			app = requests.post("https://my.telegram.org/apps/create",
			                    data=AppInfo,
			                    cookies=cookie).text
			print(app)
			print("[i] Proqram uğurla yaradıldı!")
			print("[i] API ID/HASH alınır...")
			newapp = requests.get("https://my.telegram.org/apps",
			                      cookies=cookie).text
			...
			print("[i] Məlumatlar gətirildi! Bunları qeyd edin!.\n\n")
			print(f"[i] API ID: {app_id}")
			print(f"[i] API HASH: {api_hash}")
			try:
				stringonay = int(
				    input(
				        "[?] String almaq istəyirsən? [Almaq üçün 1 yazın]: "
				    ))
			except:
				print("[!] Xahiş olunur sadəcə rəqəm yazın!")

			if stringonay == 1:
				client = InteractiveTelegramClient(StringSession(), app_id,
				                                   api_hash, numara)
			...
			print(
			    "[i] Proqramınız artıq var. Kodlar alınır"
			)
